    ...
    <main className="flex flex-1 flex-col gap-6">
      <header className="flex items-start justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-[var(--fp-wood-dark)]" dir="auto">
            {user.username}
          </p>
        </div>
        <LocaleSwitcher />
      ...
      <section className="space-y-3">
        ...
        {games.length === 0 ? (
          <p className="rounded-xl bg-[var(--fp-parchment)]/60 px-4 py-8 text-center text-[var(--fp-wood-dark)]">
            {t("empty")}
          </p>
        ) : (
                  ...
                  <span className="flex items-center gap-2 text-sm">
                    <span
                      className={
                        g.status === "open"
                          ? "text-[var(--fp-win)]"
                          : "text-[var(--fp-wood-dark)]"
                      }
                    >
                      {g.status === "open" ? t("statusOpen") : t("statusClosed")}
                    </span>
                    <span className="text-[var(--fp-wood-dark)]">{tf(g.createdAt)}</span>
                  </span>
                </Link>
              </li>
            ))}
