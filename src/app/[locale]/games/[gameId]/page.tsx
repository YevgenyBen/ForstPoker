    ...
    <main className="flex flex-1 flex-col gap-6 pb-4">
      <header className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <Link
            href={`/${locale}/games`}
            className="text-sm text-[var(--fp-felt)] underline"
          >
            ← {t("title")}
          </Link>
          ...
          <p className="text-sm text-[var(--fp-wood-dark)]">
      ...
      <section className="flex flex-wrap items-center gap-3 rounded-xl border border-[var(--fp-wood-mid)]/25 bg-[var(--fp-panel)] p-4">
        ...
        {!isMember && (
          <p className="text-sm text-[var(--fp-wood-dark)]">{t("nonMemberHint")}</p>
        )}
      </section>
        ...
        <ul className="space-y-1 rounded-xl bg-[var(--fp-parchment)]/50 p-3">
          {members.map((m) => (
            <li key={m.userId} className="flex justify-between text-sm" dir="auto">
              ...
              <span className="text-[var(--fp-wood-dark)]">{tf(m.joinedAt)}</span>
            </li>
          ))}
        </ul>
      </section>
      ...
      <section>
        <h2 className="mb-2 font-semibold">{t("ledger")}</h2>
        <div className="overflow-x-auto rounded-xl border border-[var(--fp-wood-mid)]/25">
          <table className="w-full min-w-[320px] text-sm">
            <thead className="bg-[var(--fp-parchment)]">
              <tr>
                <th className="px-2 py-2 text-start">{t("members")}</th>
                <th className="px-2 py-2 text-start">type</th>
                <th className="px-2 py-2 text-end">{t("amountNis")}</th>
                <th className="px-2 py-2 text-start">{t("note")}</th>
                <th className="px-2 py-2 text-start">time</th>
              </tr>
            </thead>
            <tbody>
              {ledger.map((row) => (
                <tr key={row.id} className="border-t border-[var(--fp-wood-mid)]/15">
                  ...
                  <td className="whitespace-nowrap px-2 py-2 text-[var(--fp-wood-dark)]">
                    {tf(row.recordedAt)}
                  </td>
                </tr>
              ))}
      ...
      {game.status === "closed" && (
        <section>
          <h2 className="mb-2 font-semibold">{t("settlements")}</h2>
          {settlements.length === 0 ? (
            <p className="text-sm text-[var(--fp-wood-dark)]">—</p>
          ) : (
          ...
          {closerName && (
            <p className="mt-2 text-sm text-[var(--fp-wood-dark)]">
